...
CREATE TABLE telefone(
  ...
  ID_cont INT,
  CONSTRAINT fk_tel_cont
    FOREIGN KEY (ID_cont)
    REFERENCES contato(ID_cont)
    ON DELETE CASCADE
);
...
CREATE TABLE email (
  ...
  TIPO VARCHAR(20) NOT NULL,
  fk_ID_cont INT,
  CONSTRAINT fk_ema_cont
    FOREIGN KEY (fk_ID_cont)
    REFERENCES contato(ID_cont)
    ON DELETE CASCADE
);
...
INSERT INTO email (ENDERECO, TIPO, fk_ID_cont) VALUES
...
SELECT c.ID_cont, c.NOME, t.NUMERO
FROM contato c
...
EXPLAIN SELECT * FROM contato WHERE nome='Ana';
